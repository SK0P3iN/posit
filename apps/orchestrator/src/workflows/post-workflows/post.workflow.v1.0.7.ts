...
export async function postWorkflowV107({
  ...
  // Prefer a short human string over Temporal ActivityFailure JSON in post.error.
  ...
  // published - mark ERROR with UNCONFIRMED: so Retry stays blocked until the
  // user confirms already-live (or we reconcile) and they do not duplicate it.
      ...
      try {
        ...
        if (handle.type === 'bad-body') {
          await changeState(
            postsList[0].id,
            'ERROR',
            handle.message || 'Platform rejected the post',
            postsList
          );
          await inAppNotification(
            post.organizationId,
      ...
      try {
        ...
        const errorMessage =
          handle.type === 'bad-body'
            ? handle.message || 'Platform rejected the post'
            : handle.type === 'stop'
            ? handle.message || 'Refresh channel needed'
            : formatPublishError(err, 'Failed to publish post');
        await changeState(
          postsList[0].id,
          'ERROR',
          errorMessage,
